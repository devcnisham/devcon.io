...
| 42 | A git config include outside the repo kills every git check | **open** | Found by CI on 2026-08-11, bumping `actions/checkout` to v7. v6+ persists the token to `$RUNNER_TEMP` and leaves an `includeIf` in `.git/config` pointing at it; the sandbox denies everything outside the repo, so *every* git invocation exits 128 with "unable to access …: Operation not permitted" and reads like a broken repo. **CI is fixed with `persist-credentials: false`** — the right fix, since allowing that path would hand a sandboxed check the `GITHUB_TOKEN`. **The product hole is still open:** a normal Mac with an `includeIf` in `~/.gitconfig` — the standard work/personal split — hits the identical failure against a real project, and no test covers it. Candidate fix is `GIT_CONFIG_NOSYSTEM=1` plus `GIT_CONFIG_GLOBAL=/dev/null` in `check.ts`, which makes git checks depend on the repo alone; the cost is that a user's legitimate global config (`safe.directory`) stops applying. **Needs a decision, and a test that fails first.** |
